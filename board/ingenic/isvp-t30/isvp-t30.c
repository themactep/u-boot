...
/*
 * Ingenic ISVP-T30 board (DDR2 64 MB, SFC NOR)
 *
 * U-Boot-proper board glue: DRAM size, USB PHY bring-up. The SPL
 * (mach-xburst/t30) brings up console + PLL + Innophy DDR2 (T30N
 * 64 MB M14D5121632A).
 *
...
int dram_init(void)
{
	gd->ram_size = 64 << 20;	/* T30N: DDR2 M14D5121632A 64 MB */
	return 0;
}
 ...
 */
static void t30_usb_phy_init(bool device)
{
...
{
	(void)dev;
	t30_usb_phy_init(true);		/* DEVICE_ONLY_MODE for DFU/g_dnl */
}
...
int board_init(void)
{
	...
	/*
	 * Only the host build (dr_mode="host", t30-isvp.dts) does the
	 * host PHY bring-up here. The DFU loader (t30-isvp-dfu.dts,
...
/* Printed right after the "Model:" line; shows the exact T30 SKU. */
int checkboard(void)
{
