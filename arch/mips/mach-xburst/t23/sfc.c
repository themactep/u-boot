...
/*
 * Ingenic T23 SPL SPI-NOR loader: read U-Boot proper from SFC NOR
 * into DRAM, LZMA-decompress and jump.
 *
 * Faithful transliteration of the lean vendor NOR path (the same
 * code base as t31/sfc.c: jz_sfc_readl/writel, ssi_clk_set_rate,
 * sfc_read_data, sfc_read, sfc_init, sfc_nor_load). The SFC block
 * and the SSI cgu entry are identical to T31 (same SFC_BASE, same
 * CPM_SSICDR, same MPLL 1200 source). DRAM is already up
 * (sdram_init() ran) so the DRAM scratch/heap/load addresses are
 * valid.
 *
...
/*
 * Flash layout for the isvp_t23 NOR profile (same as T31):
 *   flash 0            : SPL (boot header + SPL code)
 *   flash UBOOT_OFFSET : U-Boot proper image (run at 0x80100000)
 *
 * DRAM scratch/heap addresses fit the 64 MB T23/T23N (and the
 * 32 MB T23DL/DN: load 0x80100000, heap 0x80a00000, scratch
 * 0x81000000 are all < 0x82000000 for a small U-Boot).
 */
#define T23_UBOOT_OFFSET	0x10000		/* CONFIG_UBOOT_OFFSET */
#define T23_UBOOT_LOAD_ADDR	0x80100000	/* CONFIG_TEXT_BASE */
#define T23_IH_MAGIC		0x27051956
#define T23_IH_HDR_LEN		64
#define T23_UBOOT_SCRATCH	0x81000000	/* compressed image in DRAM */
#define T23_SPL_HEAP_BASE	0x80a00000	/* LZMA decoder malloc pool */
#define T23_SPL_HEAP_SIZE	0x00200000	/* 2 MiB (1 MiB dict + state) */
#define T23_UBOOT_MAX		0x00800000	/* decompressed size bound */
#define T23_DRAM_BASE		0x80000000	/* KSEG0 cached DRAM */
#define T23_DRAM_MAX		0x08000000	/* 128 MB (covers all variants) */

/* SSI/SFC clock target, from vendor sfc_init(): clk_set_rate(SSI, 70M) */
#define T23_SSI_RATE		70000000U
/* T23N MPLL is 1200 MHz (t23/pll.c), the SSI cgu source - as on T31. */
...
extern void t23_spl_puts(const char *s);
...
void t23_spl_load_uboot(void)
{
	 ...
	 */
	if (ih_size == 0 || ih_size > T23_UBOOT_MAX) {
		t23_spl_puts("SPL: U-Boot image size out of range\n");
		return;
	}
	if (ih_load < T23_DRAM_BASE ||
	    ih_load >= T23_DRAM_BASE + T23_DRAM_MAX) {
		t23_spl_puts("SPL: U-Boot load addr out of range\n");
		return;
	}

	/* Header + compressed payload into DRAM scratch. */
	sfc_nor_load(T23_UBOOT_OFFSET, T23_IH_HDR_LEN + ih_size,
		     T23_UBOOT_SCRATCH);
	 ...
	 */
	mem_malloc_init(T23_SPL_HEAP_BASE, T23_SPL_HEAP_SIZE);
	gd->flags |= GD_FLG_FULL_MALLOC_INIT;

	out_len = T23_UBOOT_MAX;
	ret = lzmaBuffToBuffDecompress((unsigned char *)(uintptr_t)ih_load,
				       &out_len, scratch + T23_IH_HDR_LEN,
				       (SizeT)ih_size);
	if (ret) {
		char m[2] = { (char)('0' + (ret & 7)), 0 };
		t23_spl_puts("SPL: U-Boot LZMA decompress failed, SZ_ERROR=");
		t23_spl_puts(m);
		t23_spl_puts("\n");
		return;
	}
