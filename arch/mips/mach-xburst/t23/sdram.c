...
/*
 * Ingenic T23 DDR2 controller and Innophy PHY init (SPL)
 *
 ...
 * isvp_t23 profile: DDR2 M14D1G1664A, 128 MB, 16-bit bus, CS0 only,
 * DDR clock 650 MHz (MPLL 1300 MHz / 2).
 *
 * Identical Innophy DDR2 init to T31 (same PHY + same DDR2 part); the
 * only T23 delta is the clock target (mach/t23-ddr.h). The register
 * write order, poll loops and delays are timing-critical and
 * reproduced exactly from the vendor source.
 *
 ...
 * clk_set_rate(). Source is MPLL (1300 MHz), target 650 MHz, so
...
static void ddr_inno_phy_init(void)
{
	...
	while (!(readl((void __iomem *)DDR_APB_PHY_INIT) & (1 << 1)))	/* init_complete */
		;
	while (!readl((void __iomem *)T23_INIT_COMP))
		;
	writel(0, (void __iomem *)REG_DDR_CTRL);
