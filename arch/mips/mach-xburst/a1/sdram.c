 ...
 *   A1A  - external DDR (up to 2 GB); reuses the A1X profile for now
 *
 * Only A1N is validated on real silicon; A1NT/A1X/A1L are build-only
 * faithful vendor transcriptions. The A1A SoC takes external DDR up
 * to 2 GB; until A1A hardware is in hand it reuses the A1X 512 MB
 * profile (A1A keeps its own APLL 1200 MHz).
 */
...
/*
 * A1X, and A1A as a placeholder: the A1A SoC takes external DDR up to
 * 2 GB, but until A1A hardware is validated it reuses this A1X profile
 * (512 MB DDR3-700, 32-bit). A1A still runs its own APLL 1200 MHz via
 * CONFIG_A1_APLL_MHZ.
 */
#elif defined(CONFIG_A1_VARIANT_A1X) || defined(CONFIG_A1_VARIANT_A1A)
static const struct a1_ddr_variant a1_ddr = {
	.cfg = 0x4a004a35,
	.tim = { 0x07120c08, 0x0809060b, 0x030a040a, 0x20261b06, 0x80058054 },
