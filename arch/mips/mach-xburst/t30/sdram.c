...
/*
 * Ingenic T30 DDR2 controller and Innophy PHY init (SPL)
 *
 ...
 * isvp_t30 (T30N) profile: DDR2 M14D5121632A, 64 MB, 16-bit bus,
 * single CS0, 4-bank, DDR clock 450 MHz (MPLL 900 / 2).
 *
 * Identical Innophy DDR2 init to T31/T23 (same PHY + same 64 MB
 * part as T23); the only T30 delta is the clock target and the
 * 450 MHz timing/MR0 set in mach/t30-ddr.h. The register write
...
static void ddr_inno_phy_init(void)
{
	...
	while (!(readl((void __iomem *)DDR_APB_PHY_INIT) & (1 << 1)))	/* init_complete */
		;
	while (!readl((void __iomem *)T30_INIT_COMP))
		;
	writel(0, (void __iomem *)REG_DDR_CTRL);
