...
/*
 * Ingenic T30 SPL SPI-NOR loader: read U-Boot proper from SFC NOR
 ...
 * MPLL source rate differs (T30N MPLL = 900 MHz). DRAM is already
...
/*
 * Flash layout for the isvp_t30 NOR profile (same as T31/T23):
 *   flash 0            : SPL (boot header + SPL code)
 *   flash UBOOT_OFFSET : U-Boot proper image (run at 0x80100000)
 *
 * DRAM scratch/heap addresses fit the 64 MB T30N (load 0x80100000,
 * heap 0x80a00000, scratch 0x81000000).
 */
#define T30_UBOOT_OFFSET	0x10000		/* CONFIG_UBOOT_OFFSET */
#define T30_UBOOT_LOAD_ADDR	0x80100000	/* CONFIG_TEXT_BASE */
#define T30_IH_MAGIC		0x27051956
#define T30_IH_HDR_LEN		64
#define T30_UBOOT_SCRATCH	0x81000000	/* compressed image in DRAM */
...
/* T30L MPLL is 1000 MHz (t30/pll.c), the SSI cgu source. */
#define T30_MPLL_RATE		1000000000U
...
{
	unsigned int pll_rate = T30_MPLL_RATE;
	unsigned int rate = T30_SSI_RATE;
	unsigned int cdr;
	u32 regval;
...
extern void t30_spl_puts(const char *s);
...
{
	u8 *scratch = (u8 *)T30_UBOOT_SCRATCH;
	u32 ih_magic, ih_size, ih_load;
	SizeT out_len;
	int ret;

	/* Legacy mkimage header (64 bytes, big-endian fields). */
	sfc_nor_load(T30_UBOOT_OFFSET, T30_IH_HDR_LEN, T30_UBOOT_SCRATCH);
	...
	sfc_nor_load(T30_UBOOT_OFFSET, T30_IH_HDR_LEN + ih_size,
		     T30_UBOOT_SCRATCH);
	 ...
	 */
	mem_malloc_init(T30_SPL_HEAP_BASE, T30_SPL_HEAP_SIZE);
	gd->flags |= GD_FLG_FULL_MALLOC_INIT;

	out_len = T30_UBOOT_MAX;
	ret = lzmaBuffToBuffDecompress((unsigned char *)(uintptr_t)ih_load,
				       &out_len, scratch + T30_IH_HDR_LEN,
				       (SizeT)ih_size);
	if (ret) {
		char m[2] = { (char)('0' + (ret & 7)), 0 };
		t30_spl_puts("SPL: U-Boot LZMA decompress failed, SZ_ERROR=");
		t30_spl_puts(m);
		t30_spl_puts("\n");
		return;
	}
