 ...
 * dictionary is the output buffer); t41_spl_load_uboot() points
 ...
 *
 * SPL grew to ~38 KB once DM-in-SPL landed (DM core + libfdt + RAM
 * uclass + per-variant tables). 0x10000 (64 KB) gives the SPL the
 * full vendor BIG_SPL ceiling (100 KB) of headroom while still
 * sector-aligned. MUST stay in sync with the U-Boot entry offset in
 * the binman image (arch/mips/dts/t41-isvp-u-boot.dtsi).
 */
#define T41_UBOOT_OFFSET	0x10000		/* must match binman u-boot @ */
#define T41_UBOOT_LOAD_ADDR	0x80100000	/* CONFIG_SYS_TEXT_BASE */
/*
 * U-Boot proper is a gzip payload wrapped in a 64-byte legacy mkimage
 * header at T41_UBOOT_OFFSET. Read the header for the exact compressed
...
#define T41_UBOOT_SCRATCH	0x81000000	/* compressed image in DRAM */
#define T41_SPL_HEAP_BASE	0x80a00000	/* LZMA decoder malloc pool */
#define T41_SPL_HEAP_SIZE	0x00200000	/* 2 MiB (1 MiB dict + state) */
#define T41_UBOOT_MAX		0x00800000	/* decompressed size bound */
#define T41_DRAM_BASE		0x80000000	/* KSEG0 cached DRAM */
#define T41_DRAM_MAX		0x4000000	/* 512 MB A1N */
...
{
	unsigned int pll_rate = T41_MPLL_RATE;
	unsigned int rate = T41_SFC_RATE * 4;
	unsigned int cdr;
	u32 regval;
...
extern void t41_spl_puts(const char *s);
...
t41_jump_to_uboot(unsigned long target)
{
 ...
 */
void t41_spl_load_uboot_with(int (*read_fn)(u32 src, u32 cnt, u32 dst))
{
	...
	if (read_fn(T41_UBOOT_OFFSET, T41_IH_HDR_LEN, T41_UBOOT_SCRATCH)) {
	...
	if (read_fn(T41_UBOOT_OFFSET, T41_IH_HDR_LEN + ih_size,
		    T41_UBOOT_SCRATCH)) {
	 ...
	 * flush without hanging. t41_jump_to_uboot() enters via KSEG1.
	 */
...
{
	t41_spl_load_uboot_with(sfc_nor_load_wrapper);
}
