...
/*
 * Ingenic T20 DDR2 controller and Synopsys DWC PHY init (SPL)
 *
 * Faithful transliteration of the vendor known-good DWC DDR2 path
 * (arch/mips/cpu/xburst/ddr_dwc.c + t20/ddr_set_dll.c + the DDR
 * branch of t20/clk.c clk_set_rate) for the isvp_t20 (T20N)
 * profile: DDR2 M14D5121632A, 64 MB, 16-bit, single CS0, 4-bank,
 * DDR clock 500 MHz (MPLL 1000 / 2, so cdr = 1, NOT bypass).
 *
 * T20 is the only XBurst1 camera SoC that is NOT Innophy: it uses
 ...
 * mach/t20-ddr.h (exact ddr_params_creator output).
 *
 ...
 * cdr = ((pll + rate - 1) / rate - 1) & 0xff = 1. The T20 CGU
