...
/*
 * SPL memory layout. T21 boots cache-as-RAM (16 KB L1 + 64 KB L2 =
 * ~80 KB, same as T23): there is NO backed memory until the SPL itself
 * brings DDR up - only the cache-resident footprint (image + BSS + the
 * live top of the stack) exists. board_init_f() inits DDR imperatively
 * and makes the SPL DRAM-resident (image re-read from NOR, live data
 * copied from the cache) before spl_init(), so by the time anything
 * allocates from the malloc-f arena these are real DRAM addresses:
